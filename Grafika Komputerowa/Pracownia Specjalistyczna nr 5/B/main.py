...
def percent_black_selection():
    ...
    if right_img_label and current_image:
        percent_value = float(
            percent_entry.get()
        )  # Pobranie wartości procentowej selekcji czarnego z pola wprowadzania
...
manual_threshold_button = tk.Button(
    root, text="Binaryzacja ręczna", command=manual_thresholding
)
...
percent_black_selection_button = tk.Button(
    root, text="Procentowa selekcja czarnego", command=percent_black_selection
)
percent_black_selection_button.grid(row=5, column=1)

mean_iterative_selection_button = tk.Button(
    root, text="Selekcja iteratywna średniej", command=mean_iterative_selection
)
